...
public class FlechetteToxicRenderer extends EntityRenderer<FlechetteToxicEntity, ProjectileRenderState> {
    private FlechetteToxicModel model;

    private static final Identifier FLECHETTE_TOXIC_TEXTURE =
            Identifier.fromNamespaceAndPath(OperationKnightfall.MODID, "textures/entity/flechette_toxic.png");
    ...
    public void submit(ProjectileRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        collector.submitModelPart(model.root(), poseStack, RenderTypes.entitySolid(FLECHETTE_TOXIC_TEXTURE), packedLight, OverlayTexture.NO_OVERLAY, null);

        poseStack.popPose();
        super.submit(state, poseStack, collector, camera);
    }

    public Identifier getTextureLocation(FlechetteToxicEntity entity) {
    ...
    @Override
    public void extractRenderState(FlechetteToxicEntity entity, ProjectileRenderState state, float partialTick) {
    ...
    @Override
    protected net.minecraft.world.phys.AABB getBoundingBoxForCulling(FlechetteToxicEntity entity) {
