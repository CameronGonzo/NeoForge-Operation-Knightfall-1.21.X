...
public class IonTippedSteelSlugRenderer extends EntityRenderer<IonTippedSteelSlugEntity, ProjectileRenderState> {
    private IonTippedSteelSlugModel model;

    private static final Identifier ION_TIPPED_STEEL_SLUG_TEXTURE =
            Identifier.fromNamespaceAndPath(OperationKnightfall.MODID, "textures/entity/ion_tipped_steel_slug.png");
    ...
    public void submit(ProjectileRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        collector.submitModelPart(model.root(), poseStack, RenderTypes.entitySolid(ION_TIPPED_STEEL_SLUG_TEXTURE), packedLight, OverlayTexture.NO_OVERLAY, null);

        poseStack.popPose();
        super.submit(state, poseStack, collector, camera);
    }

    public Identifier getTextureLocation(IonTippedSteelSlugEntity entity) {
    ...
    @Override
    public void extractRenderState(IonTippedSteelSlugEntity entity, ProjectileRenderState state, float partialTick) {
    ...
    @Override
    protected net.minecraft.world.phys.AABB getBoundingBoxForCulling(IonTippedSteelSlugEntity entity) {
