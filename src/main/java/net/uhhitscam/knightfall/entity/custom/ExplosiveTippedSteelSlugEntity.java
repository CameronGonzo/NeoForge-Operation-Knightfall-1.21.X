...
public class ExplosiveTippedSteelSlugEntity extends Snowball {
    ...
    public ExplosiveTippedSteelSlugEntity(EntityType<? extends ExplosiveTippedSteelSlugEntity> entityType, Level level) {
    ...
    public ExplosiveTippedSteelSlugEntity(EntityType<? extends ExplosiveTippedSteelSlugEntity> entityType, Level level, LivingEntity shooter, float slugSpeed, int slugDamage, WeaponClassification classification, WeaponName weaponName) {
        super(entityType, level);
    ...
    protected Item getDefaultItem() {
        return ModItems.EXPLOSIVE_TIPPED_STEEL_SLUG.get();
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        int i = 1;
        int explosiveTippedSteelSlugDamage = i + slugDamage;

        if (net.uhhitscam.knightfall.util.WeaponDamage.hurt(entity, this.damageSources().thrown(this, this.getOwner()), explosiveTippedSteelSlugDamage)) {
...
//                level().playSound((Player) null, entity.getX(), entity.getY(), entity.getZ(), blasterFireSound, SoundSource.NEUTRAL, 0.5F, 1.0F);
    ...
    protected void onHit(HitResult result) {
        ...
        if (!this.level().isClientSide()) {
            CustomExplosion.create(this, result.getLocation(), 4, 4F, 0.2, 1, Level.ExplosionInteraction.BLOCK, false);
