...
public class FlechetteToxicSpreadCanEntity extends Snowball {
    private final float flechetteSpeed;
    private final int flechetteDamage;
    private boolean hasExploded = false;

    public FlechetteToxicSpreadCanEntity(EntityType<? extends FlechetteToxicSpreadCanEntity> entityType, Level level) {
        super(entityType, level);
        this.flechetteSpeed = 2.0F;
        this.flechetteDamage = 0;
    }

    public FlechetteToxicSpreadCanEntity(EntityType<? extends FlechetteToxicSpreadCanEntity> entityType, Level level, LivingEntity shooter, float flechetteSpeed, int flechetteDamage) {
        super(entityType, level);
    ...
    protected Item getDefaultItem() {
        return ModItems.FLECHETTE_TOXIC_SPREAD_CANISTER.get();
    }
...
//            }
    ...
    private void spawnToxicFlechettes() {
        if (!(this.level() instanceof ServerLevel serverLevel)) return;

        for (int i = 0; i < 10; i++) {
            FlechetteToxicEntity flechette = new FlechetteToxicEntity(ModEntities.FLECHETTE_TOXIC.get(), serverLevel, (LivingEntity) this.getOwner(), 0, flechetteDamage, 2);
