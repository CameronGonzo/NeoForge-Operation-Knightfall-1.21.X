...
public class TibannaXBlasterBoltEntity extends Snowball {
    ...
    public TibannaXBlasterBoltEntity(EntityType<? extends TibannaXBlasterBoltEntity> entityType, Level level, LivingEntity shooter, float bolt_speed, int blasterDamage, WeaponClassification classification, WeaponName weaponName, boolean explosiveShot, boolean concussiveShot) {
        super(entityType, level);
    ...
    protected void onHit(HitResult result) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (this.level() instanceof ServerLevel serverLevel) {
                if (explosiveShot) {
                    serverLevel.sendParticles(ModParticles.EXPLOSIVE_SHOT_TIBANNAX_PARTICLES.get(),
                            this.getX(), this.getY(), this.getZ(),
                            1, 0, 0, 0, 0);
                } else {
