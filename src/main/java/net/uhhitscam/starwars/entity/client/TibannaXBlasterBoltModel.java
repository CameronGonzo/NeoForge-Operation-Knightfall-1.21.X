...
public class TibannaXBlasterBoltModel extends HierarchicalModel<TibannaXBlasterBoltEntity> {
    private final ModelPart tibannaxblasterbolt;

    public TibannaXBlasterBoltModel(ModelPart root) {
        this.tibannaxblasterbolt = root.getChild("tibannax_blaster_bolt");
    }
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition tibannax_blaster_bolt = partdefinition.addOrReplaceChild("tibannax_blaster_bolt", CubeListBuilder.create().texOffs(0, 0).addBox(-16.0F, -0.75F, 0.0F, 30.0F, 1.0F, 1.0F, new CubeDeformation(0.0F))
    ...
    @Override
    public void setupAnim(TibannaXBlasterBoltEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {

    }

    @Override
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, int color) {
        tibannaxblasterbolt.render(poseStack, vertexConsumer, packedLight, packedOverlay, color);
    }

    @Override
    public ModelPart root() {
        return tibannaxblasterbolt;
    }
}
